//
//  UIColor_Extension.swift
//  Box
//
//  Created by Jun Seub Lim on 30/06/2019.
//  Copyright © 2019 Jun Seub Lim. All rights reserved.
//
import UIKit
extension UIColor {
    @nonobjc class var darkGrey: UIColor {
        return UIColor(red: 33.0 / 255.0, green: 37.0 / 255.0, blue: 41.0 / 255.0, alpha: 1.0)
    }
    
    @nonobjc class var pumpkinOrange: UIColor {
        return UIColor(red: 1.0, green: 116.0 / 255.0, blue: 0.0, alpha: 1.0)
    }
    @nonobjc class var lightBlueGrey: UIColor {
        return UIColor(red: 206.0 / 255.0, green: 212.0 / 255.0, blue: 218.0 / 255.0, alpha: 1.0)}
    @nonobjc class var charcoalGrey: UIColor {
            return UIColor(red: 52.0 / 255.0, green: 58.0 / 255.0, blue: 64.0 / 255.0, alpha: 1.0)
        }
    @nonobjc class var blueGrey10: UIColor {
        return UIColor(red: 142.0 / 255.0, green: 142.0 / 255.0, blue: 147.0 / 255.0, alpha: 0.1)
    }
    @nonobjc class var blueGrey: UIColor {
        return UIColor(red: 134.0 / 255.0, green: 142.0 / 255.0, blue: 150.0 / 255.0, alpha: 1.0)
    }
    @nonobjc class var lightBlueGreyTwo: UIColor {
        return UIColor(red: 199.0 / 255.0, green: 199.0 / 255.0, blue: 204.0 / 255.0, alpha: 1.0)
    }
    @nonobjc class var paleGrey: UIColor {
        return UIColor(red: 248.0 / 255.0, green: 249.0 / 255.0, blue: 250.0 / 255.0, alpha: 1.0)
    }
    @nonobjc class var brownishGrey: UIColor {
        return UIColor(white: 112.0 / 255.0, alpha: 1.0)
    }
    @nonobjc class var brownGrey: UIColor {
        return UIColor(red: 165.0 / 255.0, green: 165.0 / 255.0, blue: 165.0 / 255.0,
                       alpha: 1.0)
    }
    @nonobjc class var gunmetal: UIColor {
        return UIColor(red: 73.0 / 255.0, green: 80.0 / 255.0, blue: 87.0 / 255.0, alpha: 1.0)
    }
    @nonobjc class var darkGrey35: UIColor {
        return UIColor(red: 33.0 / 255.0, green: 37.0 / 255.0, blue: 41.0 / 255.0, alpha: 0.35)
}
    @nonobjc class var veryLightPink: UIColor {
        return UIColor(white: 239.0 / 255.0, alpha: 1.0)
    }
}

